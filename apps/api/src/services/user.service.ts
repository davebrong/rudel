 ...
 */
export async function getUserMappings(orgId: string, days = 30): Promise<UserMapping[]> {
  ...
  const results = await queryClickhouse<{ user_id: string; project_path: string; sessions: number }>(query);

  // Group by user_id and extract the most common username
  const userMap = new Map<string, { username: string | null; totalSessions: number }>();
 ...
 */
export async function getUserMapping(orgId: string, userId: string, days = 30): Promise<UserMapping | null> {
  const org = escapeString(orgId);
